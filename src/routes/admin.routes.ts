...
async function sendTelegramTestResponse(req: import("express").Request, res: import("express").Response) {
  const requestedChatId =
    typeof req.body?.chatId === "string" && /^-?\d+$/.test(req.body.chatId)
      ? req.body.chatId
      : undefined;
...
// POST is used by the admin UI. GET is intentionally supported too so an
// authenticated administrator can verify the integration directly in a browser.
adminRouter.post(
  "/telegram/test",
  requireAdmin,
  asyncHandler(sendTelegramTestResponse),
);
...
adminRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const input = z.object({ chatId: z.string().regex(/^-?\d+$/) }).parse(req.body);
    const chatId = setTelegramRuntimeChatId(input.chatId);
    const test = await sendTelegramMessage(
      `✅ Ysello is now using this Telegram destination for visitor notifications.\n${new Date().toISOString()}`,
      chatId,
    );
