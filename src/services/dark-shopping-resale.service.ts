...
import type { DarkShoppingProduct } from "./dark-shopping.client.js";
...
export function darkShoppingResalePrices(
  ...
  if (
    ...
    throw new ApiError(
      400,
      "Margin must be a whole percentage between 0 and 500.",
      "DARK_SHOPPING_MARGIN_INVALID",
    );
  }
...
function remoteProductData(
  ...
  const prices = darkShoppingResalePrices(product.price, marginPercent);
  ...
  return {
    ...
    product: {
      ...
      currency: "USD",
      coverImageUrl: /^https:\/\//i.test(product.miniature)
        ? product.miniature
        : null,
      deliveryNote:
...
export async function importDarkShoppingProducts(input: {
  ...
  if (!category || !marketplaceTaxonomySlugs.has(category.slug)) {
    throw new ApiError(
      404,
      "Choose an active category from Ysello's public marketplace taxonomy.",
      "CATEGORY_NOT_FOUND",
    );
  }
  await ensureOfficialSellerProfile(input.adminId);
  const marginPercent = input.marginPercent ?? env.DARK_SHOPPING_MARGIN_PERCENT;
  ...
  for (const remoteProductId of input.remoteProductIds) {
    ...
    try {
      const data = remoteProductData(remote, marginPercent);
...
async function syncListingRecords(
  ...
  for (let offset = 0; offset < listings.length; offset += 500) {
    ...
    for (const listing of chunk) {
      ...
      try {
        const data = remoteProductData(remote, listing.marginPercent);
...
export async function updateDarkShoppingListing(
  ...
  await prisma.$transaction([
    prisma.darkShoppingListing.update({
      ...
      data: {
        isEnabled: enabled,
        marginPercent: input.marginPercent ?? listing.marginPercent,
      },
    }),
    prisma.product.update({
      where: { id: listing.productId },
  ...
  if (enabled || input.marginPercent !== undefined) {
...
export async function processDarkShoppingFulfillment(id: string) {
  ...
  try {
    ...
    if (!remoteOrderId) {
      ...
      const listing = await prisma.darkShoppingListing.findUniqueOrThrow({
        where: { id: fulfillment.listing.id },
        select: { marginPercent: true },
      });
      const currentPrices = darkShoppingResalePrices(
        currentProduct.price,
        listing.marginPercent,
      );
