import { importInBatches } from "../commerce/import-batches";
...
type DarkShoppingProduct = {
  ...
  is_manual_order_delivery: 0 | 1 | boolean | "0" | "1";
  category?: DarkShoppingCategory | null;
...
type DarkShoppingResale = {
  ...
  storage?: { ready: boolean; message?: string | null };
  balance: { balance: number | string; currency?: string | null } | null;
...
type TelegramStatus = {
  ...
  supportForwardingEnabled?: boolean;
...
const nav: NavItem[] = [
  ...
  { id: "telegram", label: "Telegram visitor alerts", icon: Send },
...
const navGroups: Array<{ label: string; icon: LucideIcon; items: Tab[] }> = [
  ...
  },
  {
    label: "Support",
    icon: ShieldCheck,
    items: ["refunds", "disputes", "chats", "tickets", "telegram"],
  },
];
...
function supplierNumber(value: unknown, fallback = 0) {
  const parsed = typeof value === "number" ? value : Number(value);
  return Number.isFinite(parsed) ? parsed : fallback;
}

function supplierManualDelivery(value: unknown) {
...
function supplierRetailRubCents(priceRub: unknown, marginPercent: number) {
  const normalizedPrice = Math.max(0, supplierNumber(priceRub));
  return Math.ceil(
    (Math.round(normalizedPrice * 100) * (100 + marginPercent)) / 100,
  );
}

function supplierBalanceLabel(balance: DarkShoppingResale["balance"]) {
  if (!balance) return "Not available";
  const amount = supplierNumber(balance.balance, Number.NaN);
...
function normalizedDarkShoppingResale(
  ...
  if (
    ...
    typeof payload.configuration.configured !== "boolean"
  ) {
    return null;
  }
  ...
  return {
    ...
    categoryMappings: Array.isArray(payload.categoryMappings)
      ? payload.categoryMappings
      : [],
    listings: Array.isArray(payload.listings) ? payload.listings : [],
    fulfillments: Array.isArray(payload.fulfillments)
      ? payload.fulfillments
      : [],
  } as DarkShoppingResale;
}
...
export function OperationsAdminPage() {
  ...
  const refreshTelegram = useCallback(async () => {
    ...
    setMessage("");
    try {
      const status = await apiRequest<TelegramStatus>("/api/admin/telegram/status");
      setTelegramStatus(status);
      const chats = await apiRequest<{ chats: TelegramChat[] }>("/api/admin/telegram/chats");
      setTelegramChats(chats.chats ?? []);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Could not load Telegram status.");
    } finally {
      setTelegramBusy("");
    }
  }, []);

  const sendTelegramTest = useCallback(async (chatId?: string) => {
    setTelegramBusy(chatId ? `test-${chatId}` : "test");
    setMessage("");
    try {
      const result = await apiRequest<{ sent: boolean; chatId?: string; reason?: string }>(
        "/api/admin/telegram/test",
        { method: "POST", body: chatId ? { chatId } : {} },
      );
      if (!result.sent) {
        throw new Error(result.reason === "not-configured" ? "Telegram bot token is not configured in Railway." : "Telegram test was not sent.");
      }
      setMessage(`Telegram test sent successfully to ${result.chatId ?? chatId ?? "configured chat"}.`);
      const status = await apiRequest<TelegramStatus>("/api/admin/telegram/status");
      setTelegramStatus(status);
    } catch (error) {
  ...
  const useTelegramChat = useCallback(async (chat: TelegramChat) => {
    setTelegramBusy(`use-${chat.id}`);
    setMessage("");
    try {
      await apiRequest<{ chatId: string }>("/api/admin/telegram/use-chat", {
        method: "POST",
        body: { chatId: chat.id },
      });
      setMessage(`Telegram destination set to ${chat.name} (${chat.id}) and test message sent.`);
      const status = await apiRequest<TelegramStatus>("/api/admin/telegram/status");
      setTelegramStatus(status);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Could not select Telegram chat.");
    } finally {
      setTelegramBusy("");
    }
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    const paths: Record<Tab, string> = {
      ...
      telegram: "/api/admin/telegram/status",
    };
    ...
    try {
      ...
      if (tab === "suppliers") {
        const resale = normalizedDarkShoppingResale(data);
        if (!resale) {
          throw new ApiError(
            "The Dark Shopping API deployment is outdated or returned an unexpected response. Redeploy the latest Railway API build.",
            502,
            "DARK_SHOPPING_DEPLOYMENT_MISMATCH",
          );
        }
        ...
        if (
          ...
          setDarkCategories(
            Array.isArray(remoteCategories.data?.items)
              ? remoteCategories.data.items
              : [],
          );
        } else {
          setDarkCategories([]);
      ...
      if (tab === "telegram") {
        ...
        const chats = await apiRequest<{ chats: TelegramChat[] }>(
          "/api/admin/telegram/chats",
        ).catch(() => ({ chats: [] as TelegramChat[] }));
        setTelegramChats(chats.chats ?? []);
      }
    } catch (error) {
      setMessage(
  ...
  async function importShop2TopupProducts() {
    ...
    try {
      const result = await importInBatches(
        shop2TopupSelectedItemIds,
        (batch) => apiRequest<{
          imported: Array<{ remoteItemId: number; name: string; updated: boolean }>;
          skipped: Array<{ remoteItemId: number; reason: string }>;
        }>("/api/admin/shop2topup/resale/import", {
          method: "POST",
          body: {
            remoteCategoryId: Number(shop2TopupCategoryId),
            remoteItemIds: batch,
            categoryId: shop2TopupAutoCategory ? undefined : shop2TopupTargetCategoryId,
            autoCategory: shop2TopupAutoCategory,
          },
        }),
        (done, total) => setMessage(`Importing SHOP2TOPUP products: ${done} of ${total} processed…`),
      );
      setMessage([
        `SHOP2TOPUP: ${result.imported.length} imported/updated, ${result.remaining.length} remaining. Products are saved as drafts.`,
        result.error,
        ...result.skipped.slice(0, 3).map((item) => `Item ${item.remoteItemId}: ${item.reason}`),
        result.remaining.length ? "Remaining items stay selected for retry." : "",
      ].filter(Boolean).join(" "));
      setShop2TopupSelectedItemIds(result.remaining);
      await load();
    } catch (error) {
      setMessage(
        error instanceof Error
  ...
  const adminResolvedCondition = adminIsSocialAccountListing
    ? adminCategorySelection?.slug?.endsWith("-new-accounts")
      ? "New"
      : adminCategorySelection?.slug?.endsWith("-old-accounts")
        ? "Aged"
  ...
  return (
    ...
    >
      ...
      <section className="ops-main">
        ...
        {tab === "telegram" ? (
          <section className="ops-grid telegram-admin-grid">
            <article className="ops-card telegram-admin-card">
              <header>
                <div>
                  <span className="store-avatar"><Send size={18} /></span>
                  <div>
                    <h2>Telegram visitor alerts</h2>
                    <p>One-click setup and testing for website visitor notifications.</p>
                  </div>
                </div>
                <Status value={telegramStatus?.configured && telegramStatus?.chatConfigured ? "READY" : "NEEDS_SETUP"} />
              </header>
              <dl>
                <div>
                  <dt>Bot</dt>
                  <dd>{telegramStatus?.configured ? `@${telegramStatus.bot?.username ?? "connected"}` : "Bot token missing"}</dd>
                </div>
                <div>
                  <dt>Destination</dt>
                  <dd>{telegramStatus?.chatId ?? "Not selected"}</dd>
                </div>
                <div>
                  <dt>Source</dt>
                  <dd>{telegramStatus?.chatSource?.replaceAll("-", " ") ?? "Unknown"}</dd>
                </div>
                <div>
                  <dt>Visitor alerts</dt>
                  <dd>{telegramStatus?.visitorNotificationsEnabled === false ? "Disabled" : "Enabled"}</dd>
                </div>
              </dl>
              <div className="row-actions telegram-admin-actions">
                <button
                  type="button"
                ...
                >
                  <Send size={15} /> {telegramBusy === "test" ? "Sending…" : "Send test message"}
                </button>
                <button
                  type="button"
                  disabled={Boolean(telegramBusy)}
                ...
                >
                  <RefreshCw size={15} className={telegramBusy === "refresh" ? "spinning" : ""} /> Detect chats
                </button>
                <a
                  className="secondary-button"
                  href="/api/admin/telegram/test"
                  target="_blank"
                  rel="noreferrer"
                >
                  Browser test
              ...
              {!telegramStatus?.configured ? (
                <p className="ops-inline-note error">
                  {telegramStatus?.tokenPresent
                    ? `Telegram rejected or could not verify the configured bot token${telegramStatus.error ? `: ${telegramStatus.error}` : "."}`
                    : "Add TELEGRAM_BOT_TOKEN to the Railway service and redeploy."}
                  {" "}The group destination is already built into this Ysello project as a fallback.
                </p>
              ) : null}
            </article>
            ...
            <article className="ops-card telegram-admin-card">
              <header>
                <div>
                  <span className="store-avatar"><MessageSquare size={18} /></span>
                  <div>
                    <h2>Detected Telegram chats</h2>
                    <p>Select a destination and Ysello immediately sends a confirmation message.</p>
                  </div>
                </div>
              </header>
              {telegramChats.length ? (
                <div className="telegram-chat-list">
                  {telegramChats.map((chat) => (
                    <div className={`telegram-chat-row ${telegramStatus?.chatId === chat.id ? "selected" : ""}`} key={chat.id}>
                      <div>
                        <strong>{chat.name}</strong>
                        <small>{chat.type} · {chat.id}{chat.username ? ` · @${chat.username}` : ""}</small>
                      </div>
                      <div className="row-actions">
                        <button
                          type="button"
                          disabled={Boolean(telegramBusy)}
                          onClick={() => void sendTelegramTest(chat.id)}
                        >
                          Test
                        </button>
                        <button
                          ...
                          className="primary-button"
                          disabled={Boolean(telegramBusy)}
                          onClick={() => void useTelegramChat(chat)}
                        >
                          {telegramStatus?.chatId === chat.id ? "Selected" : "Use this chat"}
                        </button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="empty-state">
                  <MessageSquare />
                  <strong>No recent chats returned</strong>
                  <p>The current project fallback is your supergroup -1003862484719, so visitor alerts can still work even when Telegram getUpdates has no fresh messages.</p>
                </div>
              )}
            </article>

            <article className="ops-card telegram-admin-card telegram-how-it-works">
              <h2>Visitor delivery path</h2>
              <p>Public browser → /api/visitor/notify → Railway → Telegram Bot API → PvaPrime and Website Visitor Notify.</p>
              <p>The browser never receives the bot token. IP, country headers, browser/device details, referrer and page URL are assembled on the server. Repeat visits from the same IP/browser are deduplicated for {30} minutes by default.</p>
            </article>
          </section>
        ) : null}
          ...
          <section className="dark-resale-workspace">
            ...
            {!darkResale ? (
              ...
              <div className="ops-message error">
                ...
                {darkResale.storage.message ??
                  "Run the latest Prisma migrations on the Railway API service, then redeploy."}
              </div>
            ) : !darkResale.configuration.configured ? (
              <div className="ops-message error">
                <strong>
                  ...
                  <>
                    <section className="dark-resale-import-bar">
                      ...
                      <label className="dark-resale-publish">
                        ...
                        />
                        <span>Publish selected products immediately</span>
                      </label>
                      <button
                        type="button"
                        disabled={Boolean(busy)}
                    ...
                    <section className="dark-resale-product-grid">
                      {darkProducts.map((product) => {
                        ...
                        const manualDelivery = supplierManualDelivery(
                          product.is_manual_order_delivery,
                        );
                        const minimumOrder = Math.max(
                          1,
                          Math.trunc(supplierNumber(product.minimum_order, 1)),
                        );
                        const unsupported = manualDelivery || minimumOrder > 20;
              ...
              <section className="seller-derived-account-summary">
                <div>
                  ...
                  <span>
                    ...
                    <small>
                      Platform and type come from the selected path.
                    </small>
                  </span>
                </div>
                <p>
