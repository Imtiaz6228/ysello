...
app.get("/api/visitor/session", (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  res.json({ sessionToken: createVisitorSession(req) });
});

app.post("/api/visitor/notify", asyncHandler(async (req, res) => {
  const body = req.body && typeof req.body === "object" ? req.body : {};
  const accepted = await queueVisitorTelegramBeacon(req, {
    sessionToken: typeof body.sessionToken === "string" ? body.sessionToken.slice(0, 160) : undefined,
  ...
  res.status(202).json({ accepted, reason: accepted ? "sent_or_recently_notified" : hasValidVisitorSession(req, body.sessionToken, false) ? "not_eligible_or_delivery_failed" : "invalid_session" });
}));
...
if (isProduction && fs.existsSync(frontendIndex)) {
  ...
  const listingPrice = (product: { priceCents?: number; priceUsdCents?: number; priceCnyCents?: number; priceRubCents?: number }, locale: string) => {
    const usd = product.priceUsdCents || product.priceCents || 0;
    if (locale === "zh-CN") return `¥${((product.priceCnyCents || Math.ceil(usd * 7.24)) / 100).toFixed(2)} CNY`;
    if (locale === "ru") return `₽${((product.priceRubCents || Math.ceil(usd * 91.5)) / 100).toFixed(2)} RUB`;
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const content = [
        ...
        `<section><h2>Approved marketplace listings</h2>${products.length ? links(products.map((item) => ({ path: productSeoPath(item), title: localizedProduct(item, requestLocale(req.query.lang)).name, description: `${localizedProduct(item, requestLocale(req.query.lang)).shortDescription || ""} · ${listingPrice(item, requestLocale(req.query.lang))}` }))) : "<p>No public listings are available yet.</p>"}</section>`,
        stores.length
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const localizedPrice =
        locale === "zh-CN"
          ? { cents: product.priceCnyCents || Math.ceil(priceCents * 7.24), currency: "CNY", symbol: "¥" }
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const content = `<section><h2>Products from ${escapeHtml(store.storeName)}</h2>${products.length ? links(products.map((item) => ({ path: productSeoPath(item), title: localizedProduct(item, requestLocale(req.query.lang)).name, description: `${localizedProduct(item, requestLocale(req.query.lang)).shortDescription || ""} · ${listingPrice(item, requestLocale(req.query.lang))}` }))) : "<p>This verified store has no public listings at the moment.</p>"}</section>`;
      sendHtml(
        res,
        renderSeoDocument(frontendTemplate, {
