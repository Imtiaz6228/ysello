...
export function hasValidVisitorSession(req: Request, sessionToken: unknown, requireMature = true) {
  return validVisitorSession(sessionToken, req.get("user-agent") || "", env.CSRF_SECRET, Date.now(), requireMature ? 30000 : 0);
}

export function createVisitorSession(req: Request) {
  return issueVisitorSession(req.get("user-agent") || "", env.CSRF_SECRET);
}
...
async function queueVisitor(req: Request, client: VisitorClientContext, force: boolean) {
  ...
  if (!hasValidVisitorSession(req, client.sessionToken)) return false;
  const ua = req.get("user-agent") || "";
  ...
  if ((client.dwellSeconds ?? 0) < 35 || (client.visibilitySeconds ?? 0) < 30) return false;
  try {
