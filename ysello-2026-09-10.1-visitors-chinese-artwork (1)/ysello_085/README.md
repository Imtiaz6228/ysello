...
| ------- | -------------------------------------------------------- | ---------------------------------------------- |
| `GET`   | `/api/admin/dark-shopping/resale`                        | Listings, supplier balance, category mappings, and fulfillment |
| `POST`  | `/api/admin/dark-shopping/resale/categories/import`      | Add one selected Dark Shopping category to Ysello              |
...
| `PATCH` | `/api/admin/dark-shopping/resale/listings/:id`           | Pause/enable a listing                                         |
...
the provider as URL-encoded POST parameters with the API key in the POST body,
matching Dark Shopping's documented parameter transport and avoiding long URLs.
...
1. Select a Dark Shopping category.
2. Click **Add category to Ysello** if you want that supplier category in the
   local catalog. Repeating the action is safe and reuses the existing mapping.
3. Search/load products and review supplier cost, automatic/manual delivery,
   minimum order, stock, and quality information.
4. Select only the products you want to resell.
5. Choose the destination Ysello category (an imported Dark Shopping category is
   selected automatically when a mapping exists).
...
`ceil(supplier price × 1.30)`. The global Dark Shopping supplier markup is locked
to **30%** in server-side pricing, sync, checkout validation, and fulfillment,
then rounded upward so fractional kopecks do not reduce it. USD and CNY prices
use Ysello's existing storefront rates. The supplier-cost markup is separate
from the marketplace's configured commission accounting.
...
the supplier-cost markup remains 30% in RUB.
...
a **Test connection** action, category import/mapping, listing synchronization,
pause/enable controls, supplier balance, fulfillment status, and retry actions.
